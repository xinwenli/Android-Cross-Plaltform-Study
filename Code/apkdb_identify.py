...
def dir_has_apk_tar_scan(db_dir):
    dir_has_apk_tar_names = []
    for root, dirs, files in os.walk(db_dir, topdown=False):
        for name in dirs:
            if dir_has_apk_tar(os.path.join(root, name)):              
                dir_has_apk_tar_names.append(os.path.join(root, name))
    return dir_has_apk_tar_names
...
if len(sys.argv) == 2:
    db_dir = scan_dir_preprocess(sys.argv[1])
    if db_dir == '':
        exit(1)
    db_dir_splited = db_dir.split(os.sep)
    out_dir = db_dir_splited[len(db_dir_splited)-1]
#check if output directory is speficied, otherwise create a new output folder follow by the scanning name
    ...
    print("please provide a database of APK DB or decoded TAR DB directory")
    print("Usage: python3 apkdb_identify.py [your APK DB directory or decoded TAR DB directory] (optional)[output directory name]")
    exit(0)

dirnames = dir_has_apk_tar_scan(db_dir)
for dirname in dirnames:
    print("Original APK or TAR directory: " + dirname)
